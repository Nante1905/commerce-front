  ...
  reducers: {
    ...
    setDemandeNature: (state, action) => {
      ...
      state.demandes.map((d, iDemande) => {
        if (state.checkable) state.demandes[iDemande].total = 0;
    ...
    toggleDemande: (state, action) => {
      ...
      const checked =
        state.demandes[action.payload.iDemande].details[action.payload.iDetails]
          .selected;

      state.demandes[action.payload.iDemande].details[
        action.payload.iDetails
      ].selected = !checked;
